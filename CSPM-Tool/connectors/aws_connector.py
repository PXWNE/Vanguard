...
class AWSConnector:
    def __init__(self, aws_access_key_id: str, aws_secret_access_key: str,
                 region_name: str = "us-east-1", demo_scoped: bool = False):
        self.access_key    = aws_access_key_id
        self.secret_key    = aws_secret_access_key
        self.region        = region_name
        self._session      = None
        # demo_scoped: when true, global-namespace resources (IAM users,
        # CloudTrail) are filtered to project=cspm-demo tagged / region-local
        # resources only, so multiple demo accounts sharing one real AWS
        # account don't leak each other's (or the account owner's real)
        # findings into every team's score. Never enabled for real accounts.
        self.demo_scoped   = demo_scoped
            ...
            for page in paginator.paginate():
                for user in page["Users"]:
                    ...
                    if self.demo_scoped:
                        ...
                        if tag_map.get("project") != "cspm-demo" or tag_map.get("region") != self.region:
                            continue
    ...
    def _collect_cloudtrails(self) -> list:
        try:
            ...
            result = []
            for trail in trails:
                if self.demo_scoped and trail.get("HomeRegion") != self.region:
                    continue
                try:
